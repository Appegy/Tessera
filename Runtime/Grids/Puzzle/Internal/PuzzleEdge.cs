...
namespace Appegy.Tessera
{
    ...
    // from a plain semicircle bump (no neck) to a lifted knob with a neck and overhang. The body bow
    // direction and the tab poke direction are independent random signs (both per-edge, shared by
    // both neighbours so the polyline stitches exactly).
    internal static class PuzzleEdge
    {
            ...
            Span<float2> dest)
        {
            ...
            // The tab rides on the bowed base (sBulge * yBase) but protrudes by sTab, so the bow and
            // the tab can point in/out independently. tabY is the protrusion above the base (>= 0).
            ...
            for (var k = 1; k <= ff; k++)
            {
                ...
                var x = xL + rf * math.cos(a);
                var tabY = rf + rf * math.sin(a);
                dest[idx++] = At(p0, tangent, perp, x, baseOffset + sTab * tabY);
            }
            ...
            for (var k = 1; k <= hd; k++)
            {
                ...
                var x = 0.5f + rr * math.sin(phi);
                var tabY = hh + rr * math.cos(phi);
                dest[idx++] = At(p0, tangent, perp, x, baseOffset + sTab * tabY);
            }
